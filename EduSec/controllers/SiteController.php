...
class SiteController extends Controller
{
	...
	$login = new \app\models\LoginDetails();
		if(isset($_SESSION['email']) && isset($_SESSION['password'])){
			$_POS = [
				'_csrf' => 'QktPZE91a2x0ORkzeiUtPXU8PhB/BiZVJDguCQEiBVgaZgkcHgE9Cg==' , 'LoginForm' => [
			...
			$model = new LoginForm();
			$model->username = $_SESSION['email'];
			$model->password =$_SESSION['password'];
			$model->rememberMe =true;
			$log = \app\models\User::find()->where(['user_login_id' => $_POS['LoginForm']['username'], 'is_block' => 0])->one();
			if(empty($log)) {
	...
	if ($model->load(Yii::$app->request->post())) {
		...
		$login->user_ip_address=$_SERVER['REMOTE_ADDR'];
		$login->save(false);
		$_SESSION['email']= $log['user_id'];
		$_SESSION['password']= $log['user_password'];
		if($model->login()) {
			if(!isset(Yii::$app->request->cookies['language'])) {
				return $this->redirect(['language']);
			} else
